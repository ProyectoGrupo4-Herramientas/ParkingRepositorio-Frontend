...
const Modal = ({ isOpen, onClose, onSubmit, type }) => {
  ...
  const handleSubmit = (e) => {
    ...
    onSubmit({
      ...
      vehiculo: vehiculo || (type === 'OCR' ? 'Desconocido' : 'Vehículo Invitado'),
      ...
      <div className="bg-white rounded-2xl w-full max-w-md overflow-hidden shadow-xl border border-slate-100">
        <div className="p-6 border-b border-slate-100 flex items-center justify-between">
          <h3 className="text-lg font-bold text-slate-900">
            {type === 'OCR' ? 'Entrada Manual (OCR Fallido)' : 'Registro de Invitados'}
          </h3>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-1">Placa / Matrícula *</label>
            <input 
              type="text" 
              value={placa}
          ...
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-1">Vehículo (Opcional)</label>
            <input 
              type="text" 
              value={vehiculo}
          ...
          {type === 'Invitado' && (
            <>
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-1">Nombre del Invitado</label>
                <input 
                  type="text" 
                  value={residente}
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-1">Unidad a Visitar</label>
                <input 
                  type="text" 
                  value={unidad}
                  onChange={(e) => setUnidad(e.target.value)}
                  placeholder="Ej. Unit 402"
                  className="w-full px-3 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-900 focus:border-transparent"
                />
              </div>
            </>
