...
export default function Topbar({ onMenuOpen, searchQuery, setSearchQuery }) {
  return (
    <header className="fixed top-0 right-0 left-0 md:left-[260px] h-16 z-40 border-b border-slate-200 bg-white flex items-center px-4 md:px-6 gap-3">
      ...
      <button className="bg-red-500 text-white px-3 py-2 rounded-lg text-sm font-semibold flex items-center gap-1.5 hover:bg-red-600">
        <span className="material-symbols-outlined" style={{ fontSize: 16 }}>
          warning
        </span>
        <span className="hidden sm:inline">Bloqueo de Emergencia</span>
      </button>
